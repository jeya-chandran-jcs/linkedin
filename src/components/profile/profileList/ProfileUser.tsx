...
export default function ProfileUser({ auth }: AuthProps) {
  ...
  return (
    <Box
      ...
      <Box sx={{ position: "absolute", left: "2vw", top: "16vh" }}>
        <Avatar src={userProfile?.photoUrl?.trim() ? userProfile?.photoUrl : "https://tse3.mm.bing.net/th/id/OIP.D0bht621G_7SlAb1CjH6IwHaGF?rs=1&pid=ImgDetMain&o=7&rm=3"} alt="https://i.pinimg.com/736x/cd/3e/d5/cd3ed5a95ef5ac1d694899a51798874b.jpg" sx={{ width: 150, height: 150, objectFit: "contain", border: "4px solid white", backgroundColor: "#f5f5f5", position: "absolute" }} />

        {auth &&
          <MovalbleIcon bgColor={"white"} hoverBg={"lightgray"} position={"relative"} top={"10vh"} left={"7.4vw"}
            onClick={() => {
              setShow(true)
      ...
      <ProfileStatus authUser={userProfile} auth={auth}/>
      {auth &&
